...
AppleScript dates are built field by field. Always set `time of d to 0` before setting hours/minutes to avoid carryover bugs.

```applescript
set d to current date
set year of d to 2026
set month of d to 8          -- integer, not month constant
set day of d to 15
set time of d to 0           -- reset first
set time of d to (9 * hours + 30 * minutes)   -- 09:30
```
...
2. When listing events, include uid in output so edits/deletes can reference it
3. Default calendar is **Рабочий** unless user specifies otherwise; use **Дни рождения** for birthdays/celebrations
4. For recurring events or complex recurrence, open Calendar.app via `open -a Calendar` — AppleScript recurrence support is limited
5. When deleting, always confirm with user first
6. After creating/editing, optionally run a list command to confirm the change took effect
...
5. **Простой шаблон создания** (без построения даты через `set year of`):
   ```bash
   osascript -e 'tell application "Calendar" to tell calendar "Дни рождения" to make new event with properties {summary:"🎂 Событие", start date:date "06/17/2026 18:30:00", end date:date "06/17/2026 21:30:00"}'
   ```
6. **Конвертация МСК → ET**: время в AppleScript задаётся в таймзоне Mac (America/New_York). Для событий в МСК вычитать 7 часов: 12:30 МСК → 05:30 в `date "MM/DD/YYYY HH:MM:SS"`.
